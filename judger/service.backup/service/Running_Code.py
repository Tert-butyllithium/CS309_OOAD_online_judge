...
class SourceListener(threading.Thread):
    ...
    def run(self):
        ...
        max_memory = -1
        startTime = time.time()
        while time.time() - startTime <= self.time_limit and self.process.status() != 'zombie':
...
def main(argv):
    ...
    print(psutil.pids())
    print(type(process.status()))
    source_listener = SourceListener(time_limit, memory_limit, process, process.memory_full_info().rss / 100)
    source_listener.start()
    # taskThread.start()
    while (not os.path.exists(runtime_result) or not source_listener_end) and process.status() != 'zombie':
